...
class DigitalReportController extends Controller
{
    public function test(Request $request)
    {
        ...
        $product = DB::table('tasks')
            ...
            ->where(function ($query) use ($request) {
                return $request->from ?
                    $query->from('tasks')->whereBetween('tasks.created_at', [$request->from . ' 00:00:00', $request->to . ' 23:59:59']) : '';
            })
            //->with('prices','colors')
            ->get();
        ...
        return view('digitalreport', compact('product', 'startDate', 'selected_id', 'countries', 'price_id', 'name_id'));
    }
