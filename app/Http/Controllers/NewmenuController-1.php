...
class NewmenuController extends Controller
{
    public function index(Request $request){
        $getAct = Activity::where('activities.source_type','=','App\Models\Task')
        ->where('activities.deleted_at','=',NULL)->join('tasks', 'activities.source_id', '=', 'tasks.id')->select('activities.text', 'tasks.title', 'users.name', 'projects.title as pt', 'activities.created_at')
        ->join('users', 'tasks.user_assigned_id', '=', 'users.id')
    ->join('projects', 'tasks.project_id', '=', 'projects.id')
    ->orderBy('activities.source_id', 'asc')
        ...
        return $request->price_id ?
               ...
               ->where( function($query) use($request){
                   return $request->color_id ?
                          ...
                          ->where( function($query) use($request){
                           return $request->taskie ?
                                  $query->from('activities')
                                  ->where('activities.source_type', '=','App\Models\Task')
                                  ->where('activities.source_id', '=',$request->taskie) : '';})
                          ->where( function($query) use($request){
                           return $request->color_from ?
                                  $query->from('activities')->where('activities.text', 'like', "%".$request->color_from."%") : '';})
               ->where(function($query) use($request){
                   return $request->from ?
...
->join('divs', 'tasks.flag', '=', 'divs.id')
    ...
    $selected_id['taskie'] = $request->taskie;
                ...
                $getDiv = Div::all();
                $getTask = Task::all();
    return view('testingg',compact('all_pro','selected_id', 'getDiv','get_task_id', 'from', 'to', 'color_id', 'color_from', 'taskie', 'getTask', 'getAct'));
    }
